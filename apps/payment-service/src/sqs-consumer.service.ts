...
export class SqsConsumerService implements OnModuleInit {
  ...
  constructor(private readonly appService: AppService) {
    ...
    this.queueUrl =
      ...
      'http://sqs.us-east-1.localhost.localstack.cloud:4566/000000000000/payment-queue.fifo';
  }
  ...
  private async consumeMessages(): Promise<void> {
    if (this.isProcessing) {
      // If still processing previous message, wait before polling again
    ...
    try {
      ...
      if (response.Messages && response.Messages.length > 0) {
        ...
        this.logger.log('Received payment processing message from SQS', {
  ...
  private async processMessage(
    ...
    if (!body || !receiptHandle) {
      this.logger.error('Invalid message - missing body or receipt handle');
      return;
    }
